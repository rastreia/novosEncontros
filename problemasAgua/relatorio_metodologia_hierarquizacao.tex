...
\begin{document}
	...
	O programa \textbf{Novos Encontros} é uma política do Governo do Estado de Minas Gerais gerida pela Diretoria de Programas de Enfrentamento à Pobreza no Campo. Dentro dele, foram executadas duas ações. A primeira oriunda do programa ``Brasil sem Miséria'' do Governo Federal e a segunda oriunda do projeto ``Sementes Presentes''. A primeira ação consiste, de maneira geral, de concessão de bolsas de auxílio produtivo e treinamento para a produção agrícola. A segunda consiste, em linhas gerais, de distribuição de kits de sementes e irrigação.
	
	O programa \textbf{Novos Encontros} converge com umas das 6 principais metas de governo da atual gestão, qual seja, o enfrentamento à pobreza no campo. A pobreza é um fenômeno que registra um vasto número de interpretações em torno do que ela realmente representa. Esta multiplicidade de conceituações está associada à amplitude temática e espacial registrada por tal fenômeno ao longo das épocas. Contudo, na tentativa de adensar essa multiplicidade de perspectivas podemos compreender a pobreza como uma condição humana marcada por privação temporária ou crônica de condições (recursos financeiros e materiais, capacidades, escolhas e segurança) necessárias à constituição de uma vida adequada e de usufruto dos direitos civis e políticos.
	
	Tendo em vista a amplitude temática expressa pela pobreza é crucial que as ações desenvolvidas para a promoção de uma sociedade mais igualitária sejam múltiplas e consorciadas. Deste modo a intersetorialidade entre os órgãos empenhados na redução da pobreza transforma-se em marca necessária.
