...
setwd("~/Documentos/CADUNICO/ListasBSM")
listaBMS <- fread("mergeBSM.csv")
names(metas) <- c('cd_ibge','MUNICIPIO','nome_regiao','ano_meta')
listaBMS <- left_join(listaBMS, metas)

listaBMSComResalvas <- fread("mergeBSM.csv")
listaBMSComResalvasUsuarioFinal <- fread("mergeBSM.csv")

setwd("~/Documentos/CADUNICO/SRE_Concatenadas")
listaDiamantina <- fread("MergeSres.csv")

names(listaBMS)[11:11] <- c('num_nis_pessoa_atual')
...
for (nome in listaBMS$BENEFICIARIO.CAIXA){
...
##################### Conferencia Nome Mae #############################

nomeMaePertenceCadunico <- NULL
for (nomeMae in listaBMS$MAE.BENEFICIARIO.CAIXA){
  nomeMaePertenceCadunico <- c(nomeMaePertenceCadunico, (is.element(nomeMae, CADUNICO$nom_completo_mae_pessoa)))
}
nomeMaePertenceCadunico
listaBMSComResalvas <- cbind(listaBMSComResalvas, nomeMaePertenceCadunico)

####Não temos nome da mae em nossas listas
#nomeMaePertenceListaSPE <- NULL
#for (nomeMae in listaBMS$`MAE BENEFICIARIO CAIXA`){
#  nomeMaePertenceListaSPE <- c(nomeMaePertenceListaSPE, (is.element(nomeMae, listaDiamantina$)))
#}
#nomeMaePertenceListaSPE
...
setwd("~/Documentos/")
write.csv(listaBMSComResalvas, file = "listaResalvas.csv")
write.csv(listaBMSComResalvasUsuarioFinal, file = "listaResalvasUserFinal.csv")
